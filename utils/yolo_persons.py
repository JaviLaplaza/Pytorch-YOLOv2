...
def postprocess(bbox_pred, iou_pred, prob_pred, im_shape, cfg, thresh=0.6, iou_thresh = 0.6,
    ...
    inds = np.where(cls_inds == 14)[0] # We only want to keep bboxes of objects of class 14 (class 14 is "person") 
    ...
    for i in range(num_classes):
        ...
        if len(inds) == 0:
            continue
        # We want to keep only objects of class 14 (class 14 is "person")
        if i != 14:
            continue
        c_bboxes = bbox_pred[inds]
        c_scores = scores[inds]
